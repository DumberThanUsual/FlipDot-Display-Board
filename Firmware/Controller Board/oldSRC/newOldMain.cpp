...
class ClockFace: public BufferProducer {
  ...
  public:
    ...
    : buffer(DISPLAY_WIDTH, DISPLAY_HEIGHT)
    {
      buffer.setFont(&Font5x7FixedWide1);
      buffer.fillScreen(false);
    ...
    bool ensureBufferValidity(bool includeInacive = false) {
      if(!getBufferValidity()) {
        if(!timeValid){
          ...
          buffer.print("--:--");
          buffer.setFont(&Font4x5Fixed);
          buffer.setCursor(28, 6);
          buffer.print("-M");
        } else {
          buffer.setFont(&Font5x7FixedWide1);
          buffer.fillScreen(false);
          buffer.setCursor(0, 7);
          buffer.print(&timeinfo, "%H:%M");
          buffer.setFont(&Font4x5FixedWide1);
          buffer.setCursor(28, 6);
          buffer.print("45%");
        }
      }
...
class SurfaceScroller: public BufferProducer {
      ...
      for(;;) {   
        ...
        if (scroller->instructionBuffer.size() > 0 && scroller->instructionBuffer[0].buffer) {
          ...
          while(!instructionComplete) {
            ...
            vTaskDelay(constrain((100.0 * ANIMATION_SCALE)/pow(remainingDistance, 1), 10, 200 * ANIMATION_SCALE)/portTICK_PERIOD_MS);
  ...
  public:
    ...
    SurfaceScroller(bool isVertical = true) 
    : vertical(isVertical)
    ...
    SurfaceScroller(BufferProducer* startingBuffer) 
    : vertical(true)
    , emptyBuffer("Inactive")
    {
...
class NumberInput: public SurfaceScroller {
...
class List: public SurfaceScroller {
...
public:
  ...
  std::vector<BufferProducer*> listItems;
  ...
  bool scroll(bool direction = true /*Down = true*/) {
    ...
    if (listPosition >= (int)listItems.size()) {
      if (wrap) {
        listPosition =  0;
        return true;  
      } else {
        listPosition = listItems.size()-1;
        return false;
      }
    }
    ...
    instruction.distance = 8;
    instruction.buffer = listItems[listPosition];
    addScrollInstrction(instruction);
    return true;
  }
  
  bool scroll(int position) {
    if (position <  (int)listItems.size()) {
      while (position != listPosition) {
        scroll(position > listPosition);
      }
      return true;
    }
    else { 
  ...
  bool set(int position) {
    if (position <  (int)listItems.size()) {
      ...
      setFrame(listItems[listPosition]);
      return true;
    }
    else {
      return false;
...
public:
  ...
  virtual BaseActivity* getDefaultActivity() = 0;
};

class ActivityManager: public SurfaceScroller {
private:
...
public:
  ...
  ActivityManager()
  : SurfaceScroller(false)
  {

  }

  void startActivity(BaseActivity* activity, bool animate = true) {
    ...
    if (animate) {
      if (currentStack == activity->parentApplication) {
        ...
        scrollInstruction.distance = DISPLAY_WIDTH + 1;
  ...
  bool goToStack(Application* appStack) {
    if (!appStack) {
      startActivity(appStack->getDefaultActivity());
      return true;
    }
...
class CountdownTimer: public Application {
  ...
  static void countdownTask(void* pvParameters) {
    ...
    activityManager.startActivity(new CountdownTimer::AlarmActivity(countdownApp), false);
  ...
  public:
    ...
    void startTimer() {
      ...
      if (!timerStarted) {
        ...
        activityManager.startActivity(new CountdownTimer::CountdownActivity(parentTimer), false);
      }
      timerStarted = true;
    }
    ...
    BaseActivity* getDefaultActivity() {
      if (timer == -1) {
        return new timerSetupActivity(this);
      }
      else {
        return new CountdownActivity(this);
      }
    }
};
...
public:
  class SettingsMenu: public BaseActivity {
    List list;
  public:

    SettingsMenu(Application* parentApplication)
    : BaseActivity(parentApplication)
    {
      list.listItems.push_back(new TextSurface("WiFi"));
      list.listItems.push_back(new TextSurface("About"));
      list.setFrame(list.listItems[0]);
    }

    bool getPixel(int x, int y) {
      return list.getPixel(x, y);
    }

    bool ensureBufferValidity(bool inclueInactive = false) {
      return list.ensureBufferValidity(inclueInactive);
    }

    bool handleInput(InputEventType inputEventType) {
      if (list.handleInput(inputEventType)) {
        return true;
      }
      switch (inputEventType)
      {
...
public:
  ...
  public:
    ...
    {
      for (int i = 0; i < 6; i++) 
        list.listItems.push_back(listItems[i]);
      list.setFrame(listItems[0]);
    }
    ...
    bool handleInput(InputEventType inputEventType) {
      ...
      case LEFT_SINGLE:
        ...
        break;
      case RIGHT_SINGLE:
        switch (list.listPosition){
          case 2:
            activityManager.goToStack(&countdownTimer);
            return true;
            break;
          case 5:
            activityManager.goToStack(&settings);
            return true;
            break;
          default:
            return false;
...
void loop() {
  ...
  if (millis() - lastTimeUpdate > 1000*updateInterval) {
    getLocalTime(&timeinfo);
    lastTimeUpdate = millis();
  }
